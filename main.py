...
Дай развёрнутый ответ от лица своего персонажа. Упомяни карты которые легли (придумай сам). Ответь на запрос, расскажи что видишь в ситуации и что ждёт впереди. Пиши живым языком одним сплошным текстом без абзацев и переносов строк, как будто диктуешь голосовое. Никаких тире ни длинных ни коротких нигде в тексте. Объём {'100-150' if age >= 35 else '200-300'} слов.

КАК ПИСАТЬ (очень важно, иначе текст выдаёт нейросеть):
- Не строй ответ по схеме вступление потом объяснение потом вывод, пиши как мысли приходят, вперемешку
- Никогда не используй слова-связки: "однако", "при этом", "таким образом", "следует отметить", "во-первых", "во-вторых", "в заключение", "подводя итог"
- Иногда обрывай мысль и перескакивай на другую, потом возвращайся: "хотя нет подожди", "ну это другое"
- Иногда повтори слово или фразу дважды подряд как будто для акцента или задумался
- Иногда упомяни конкретный прошлый случай из своей практики: "у меня недавно была похожая ситуация", "я такое видел пару раз"
- Не заканчивай красиво и подведённо, последняя фраза может быть обрывистой или вообще бытовой
- Иногда не договаривай: "ну ты понимаешь о чём я", "там в общем много всего"
- НЕ будь идеально тёплым и поддерживающим в каждой фразе, иногда просто сухо по делу, иногда чуть скептично (но не грубо)
- Сильно варьируй длину предложений: несколько коротких подряд, потом одно длинное, потом снова короткое
- Пиши специфично под эту конкретную ситуацию, не обобщай ("люди обычно", "как правило" использовать нельзя)
{typing_style}
"""
...
async def send_tarot_answer_delayed(user_id: int, tarologist: dict, user_story: str, is_flagged: bool = False):
    age = tarologist.get("age", 35)
    if age >= 57:
        delay = random.randint(4 * 60, 6 * 60)      # 4–6 мин
    elif age >= 50:
        delay = random.randint(3 * 60 + 30, 5 * 60) # 3.5–5 мин
    elif age >= 43:
        delay = random.randint(3 * 60, 4 * 60 + 30) # 3–4.5 мин
    elif age >= 35:
        delay = random.randint(2 * 60 + 30, 3 * 60 + 30) # 2.5–3.5 мин
    else:
        delay = random.randint(2 * 60, 3 * 60)      # 2–3 мин
