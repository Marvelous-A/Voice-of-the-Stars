"""GitHub webhook receiver for automatic Nebo Alert Bot deployments."""
...
def shared_setting(name: str) -> str:
    value = os.getenv(name, "").strip()
    if value:
        return value
    return str(dotenv_values("/home/bot/.env").get(name) or "").strip()


PORT = int(os.getenv("DEPLOY_PORT", "9002"))
SECRET = os.getenv("WEBHOOK_SECRET", "").encode()
REPOSITORY = os.getenv("GITHUB_REPOSITORY", "Marvelous-A/Nebo-Ryadom-Bot")
BRANCH = os.getenv("DEPLOY_BRANCH", "main")
BOT_SERVICE = os.getenv("BOT_SERVICE", "nebo-alert-bot.service")
WEBHOOK_SERVICE = os.getenv("WEBHOOK_SERVICE", "nebo-deploy-webhook.service")
ADMIN_BOT_TOKEN = shared_setting("ADMIN_BOT_TOKEN")
NOTIFY_CHAT_IDS = tuple(
    int(value.strip())
    for value in os.getenv(
        "DEPLOY_NOTIFY_CHAT_IDS", shared_setting("ADMIN_ID")
    ).split(",")
...
logger = logging.getLogger("nebo-deploy")
...
def run_command(
    args: list[str], *, cwd: Path = PROJECT_DIR, check: bool = True
...
def send_telegram_notification(message: str) -> int:
    ...
    if not NOTIFY_CHAT_IDS:
        logger.error("Deploy notification skipped: no chat ID is configured")
        return 0
...
def validate_candidate(candidate_dir: Path) -> None:
    ...
    run_command(
        [
            str(python),
            "-c",
            "import config, database, handlers, keyboards, main, models, notifier, source",
        ],
        cwd=candidate_dir,
    )


def restart_bot(previous_commit: str) -> None:
    ...
    logger.error("New version failed; rolling back to %s", previous_commit)
    run_command(["git", "reset", "--hard", previous_commit])
    run_command(["systemctl", "restart", BOT_SERVICE])
...
def deploy_once() -> None:
    ...
    ancestor = run_command(
        ["git", "merge-base", "--is-ancestor", current_commit, target_commit], check=False
    )
    ...
    with tempfile.TemporaryDirectory(prefix="nebo-deploy-") as temp_dir:
    ...
    send_telegram_notification(
        "✅ «Небо рядом» обновлён и работает\n\n"
        f"Коммит: {target_commit[:7]}\n"
        f"Изменения: {subject}\n\n"
        "Проверка: тесты пройдены.\n"
        "Сервер: новый код установлен, служба active."
    )
...
def deploy_worker() -> None:
    ...
    while True:
        ...
        try:
            ...
            send_telegram_notification(
                "❌ «Небо рядом» не удалось обновить\n\n"
...
class WebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            return
        if (
            data.get("repository", {}).get("full_name") != REPOSITORY
